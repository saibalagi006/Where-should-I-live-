...
input_data=pd.read_excel("Population_Estimate_fcst_input.xlsx", header=0,converters={'FIPS':str})
#input_data['FIPS']=input_data['FIPS'].astype(str)
...
#    """.format(state,county,fips)
    ...
    data_pd=input_data.loc[(input_data.State_Name==state)&(input_data.County_Name==county)&(input_data.FIPS==fips),['year','POP_ESTIMATE','N_POP_CHANGE','Births','Deaths','Natural_Inc','International_Mig','Domestic_Mig','Net_Mig','Residual','GQ_Estimate','Birth_Rate','Death_Rate','Natural_Inc_Rate','International_Mig_Rate','Domestic_Mig_Rate','Net_Mig_Rate']]
    ...
    y_train=data_pd.loc[(data_pd.year>=2011) &( data_pd.year<=2014),'POP_ESTIMATE']
    
    y_test=data_pd.loc[(data_pd.year>2014) &( data_pd.year<=2017),'POP_ESTIMATE']
...
writer = pd.ExcelWriter('Population_Comparison_Results.xlsx')
...
#sql3="""
#select *
#into population_model_Selection
#from insert_df
#        """
