...
result_final_columns=['state','county','fips','year','actuals','fcst_Holts','fcst_Prophet']
...
##################################Holt's Model####################################################################################    
    y_train=data_pd.loc[(data_pd.year>=2011) &( data_pd.year<=2014),'crime_total']
...
######################################################Prophet#######################################################################    
    #X_train=X[6:105]
    train_df=data_pd.loc[(data_pd.year>=2011) &( data_pd.year<=2014),['year','crime_total']]
    train_df.columns = ['ds', 'y']
    test_df=data_pd.loc[(data_pd.year>2014) &( data_pd.year<=2017),['year','crime_total']]
...
writer = pd.ExcelWriter('Crime_Comparison_Results_final.xlsx')
Result_Details_Final.to_excel(writer,'Sheet1')
writer.save()
        

print('Holt rmse is ')
print(round(np.sqrt(((Result_Details_Final.loc[:,'fcst_Holts'] - Result_Details_Final.loc[:,'actuals'])**2).mean()),2))

print('Prophet rmse is ')
print(round(np.sqrt(((Result_Details_Final.loc[:,'fcst_Prophet'] - Result_Details_Final.loc[:,'actuals'])**2).mean()),2))
