...
result_final_columns=['state','fips','year','price_to_income_rating','fcst_simple_exponential_smoothing','fcst_Holt','fcst_Holt_Winters']
...
##################################Holt's Model####################################################################################    
    y_train=data_pd.loc[(data_pd.year>=2005) &( data_pd.year<=2014),'price_to_income_rating']
...
###################################################################################################################################    
    fscl_range=['2015','2016','2017','2018']
...
writer = pd.ExcelWriter('Affordability_Comparison_Results.xlsx')
Result_Details_Final.to_excel(writer,'Sheet1')
writer.save()
        

print('Simple Exponential rmse is ')
print(round(np.sqrt(((Result_Details_Final.loc[:,'fcst_simple_exponential_smoothing'] - Result_Details_Final.loc[:,'price_to_income_rating'])**2).mean()),2))

print('Holt rmse is ')
print(round(np.sqrt(((Result_Details_Final.loc[:,'fcst_Holt'] - Result_Details_Final.loc[:,'price_to_income_rating'])**2).mean()),2))


print('Holt-winters rmse is ')
print(round(np.sqrt(((Result_Details_Final.loc[:,'fcst_Holt_Winters'] - Result_Details_Final.loc[:,'price_to_income_rating'])**2).mean()),2))
